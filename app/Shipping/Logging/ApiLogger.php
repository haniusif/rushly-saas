...
namespace App\Shipping\Logging;

use App\Shipping\Models\ShippingApiLog;
use Illuminate\Support\Facades\Log;

/**
 * Writes one row to shipping_api_logs per outbound HTTP call. Used by
 * AbstractProvider so every provider gets logging for free.
 *
 * Failure to write a log row must never throw — diagnostic infrastructure
 * shouldn't take down the actual integration.
 */
class ApiLogger
{
    public function record(array $entry): void
    {
        if (! (bool) config('shipping.logging.enabled', true)) {
            return;
        }
        ...
        try {
            ...
            Log::warning('shipping.api_log_write_failed', [
                'error'    => $e->getMessage(),
    ...
    {
        $sensitive = array_map('strtolower', (array) config('shipping.logging.sensitive_headers', []));
