...
## 2026-09-09 — RDS Express driver rebrand + driver-app fixes

Recorded by hand; the sections below this one are reconstructed from `git log` as of
2026-07-27 and do not cover it.

**`rushly-saas`** (`a013335`, branch `docs/rushly-knowledge-base`)

- The hardcoded brand in the driver WhatsApp template (`ParcelResource::toArray`,
  opening line and sign-off) and in the "Generated by" footer of the four AWB label
  templates changed from "Rushly Express" to "RDS Express". Compiled Blade views were
  cleared so the labels picked it up.
- Caveat, unchanged: `ParcelResource` is **not tenant-scoped**, so all tenants send the
  same brand. `settings()->name` is the correct source but company 9 has an empty `name`
  in `general_settings` and would render blank. See
  [09-API.md §6](09-API.md).
- Unrelated to Shopify: the embedded Shopify app is *also* publicly named
  "Rushly Express" ([14-Integrations.md](14-Integrations.md)) and was **not** renamed.

**`rushly-driver` (the Flutter driver app)** — `5c806e5`, `34e17eb`, `cc6fa37`, `4d16070`

- Rebranded **RDS Express | Driver**; navy `#253985`, orange `#E87B35`.
- `ProfileController` / `DashboardController` are `permanent: true` and constructed
  before sign-in, so their eager `onInit` fetches ran without a token and left Profile
  and Home permanently empty for the session. Both now load after sign-in and on first
  view, and are cleared on sign-out.
- The Home list never repainted — all state is `Rx` but the view used `GetBuilder` and
  the controller never calls `update()`. Marking a parcel delivered showed a success
  snackbar while the card stayed in Pending.
- The status badge was always blank: the API emits `statusName`, the Dart model read
  `status_name` (same for `deliveryType`).
- Rebuilt the payment-history and map screens; released debug-signed APKs.

**Documentation correctness.** `docs/apps/rushly-driver-app.md` and `MOBILE_APPS.md §2`
were found to describe a driver app that does not exist in `haniusif/rushly-driver`
(Riverpod/go_router, NDR, runsheet, support tickets, `/var/www/rushly-driver-app`).
Both now carry a ⚠️ Doc vs Code banner rather than being rewritten, pending a decision
