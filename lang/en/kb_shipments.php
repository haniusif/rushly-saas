...
return [
    'label'    => 'Shipments',
    'overview' => 'End-to-end shipment lifecycle: create, edit, dispatch, bulk-action, plus NDR (non-delivery) tracking and stalled-shipment investigation.',
    'sub_pages' => [

        'parcels' => [
            'icon'    => 'Package',
            'label'   => 'Shipments',
            'purpose' => 'Central hub for managing the shipment lifecycle from creation through delivery — tracks each shipment through pickup, warehouse handling, hub transfer, delivery assignment, and final delivery or return. Supports COD collections, multi-carrier 3PL integration, and WMS fulfillment workflows.',
            'pages' => [
                ...
                ['path' => 'Clone',       'desc' => 'Duplicate an existing shipment — useful for recurring orders.'],
            ...
            'cross_links' => 'Feeds the NDR module on failed attempts; escalates to Abnormal Shipments when stalled. Hands off to WMS Fulfillment when shipments carry wms_* statuses. Generates merchant invoices for settlement. Integrates with Panda / Zajel / Aramex / Jet / Logestechs for multi-carrier handoff.',
            'notes'       => 'Shipment statuses have allowed-state transitions enforced server-side. 3PL integration auto-fetches AWBs from Panda. Subscription tier gates shipment count. Wallet activation checks merchant balance before store. Bulk creation supports CSV import with row-level preview & validation.',
        ],

        'bulk-action' => [
            'icon'    => 'Wand2',
            'label'   => 'Bulk action',
            'purpose' => 'Apply a state-change operation to many shipments at once (assign 100 shipments to one courier, transfer 50 to a hub, return a batch to a merchant). Designed to cut clicks for high-volume 3PL operations.',
            'pages' => [
                ['path' => 'Bulk Action form', 'desc' => 'Paste up to 500 tracking IDs or numeric IDs; selected shipments are grouped by their current status; pick an action (assign-pickup, transfer-to-hub, assign-deliveryman, return-to-merchant, escalate-to-ndr).'],
            ...
            'cross_links' => 'Triggered from the shipment Index (checkbox row selection). Feeds into the NDR module via the escalate-to-ndr action.',
            'notes'       => 'Accepts RLxxxxxx tracking format or numeric IDs. Duplicates in the paste are de-duped automatically. Pre-apply summary shows counts grouped by status so you can see the operational impact before committing. Capped at 500 shipments per request for performance.',
        ],

        'ndr' => [
            'icon'    => 'AlertTriangle',
            'label'   => 'NDR (Non-Delivery Reports)',
            'purpose' => 'Track and resolve failed delivery attempts. Each NDR captures the failure reason, the attempting driver, and the action taken (reschedule, return to merchant, transfer to hub, or escalate). Supports up to 3 attempts per shipment before auto-escalation.',
            'pages' => [
                ...
                ['path' => 'Create',         'desc' => 'Record a failure for a shipment: failure_reason dropdown, optional driver notes / photo, next_attempt_date, deliveryman. Enforces one open NDR per shipment per day and a 3-attempt cap.'],
            ...
            'cross_links' => 'Links back to Shipment via parcel_id. Auto-escalates to Abnormal Shipments after 3 failed attempts. Driver assignment ties to Couriers. Hub assignment ties to Hubs.',
            'notes'       => 'Hard limits: max 3 attempts per shipment, one open NDR per shipment per day. Reasons come from the NdrFailureReason enum (9 values). Driver photos land in /uploads/ndr/. An NDR can carry abnormal_shipment_id to group it under an investigation.',
        ],
        ...
        'abnormal' => [
            ...
            'pages' => [
                ...
                ['path' => 'Show',         'desc' => 'Investigation page: shipment details, stale progress bar, detected_at / escalated_at / assigned_to, resolution_note log, and action buttons (assign, create NDR, log contact, escalate, close-as-lost, resolve). 15 most-recent shipment events shown inline.'],
            ...
            'cross_links' => 'Detection triggers off ParcelEvent gaps; auto-creates from NDR after 3 failed attempts. Links back to Shipment for the full event timeline. Resolution feeds back into Shipment status history.',
