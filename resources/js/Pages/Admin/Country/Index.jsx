...
export default function Index({ rows = [], pagination = {}, filters = {}, permissions = {}, urls = {}, t = {} }) {
    const del = (r) => {
    ...
    const cols = permissions.update || permissions.delete ? 7 : 6;
    ...
    return (
        <AdminLayout title={t.title} breadcrumbs={[t.title, t.list]}>
            ...
            <FilterBar indexUrl={urls.index} filters={filters} t={t} />
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-border bg-muted/30 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                                    ...
                                    <th className="px-4 py-3 text-end">{t.cities}</th>
                            ...
                            <tbody>
                                ...
                                {rows.map((r, idx) => (
                                    <tr key={r.id} className="border-b border-border align-top last:border-0 hover:bg-muted/20">
                                        ...
                                        <td className="px-4 py-3"><span className="font-mono text-xs">{r.code || '—'}</span></td>
                                        <td className="px-4 py-3 text-end tabular-nums">{r.cities_count}</td>
