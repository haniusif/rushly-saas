...
import { Tags, Plus, Edit, Trash2, MoreVertical, ChevronLeft, ChevronRight, Lock } from 'lucide-react';
    ...
    return (
        <AdminLayout title={t.title} breadcrumbs={[t.title, t.list]}>
            ...
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-muted-foreground"><Tags className="h-4 w-4" /><span>{showing}</span></div>
                {permissions.create && (
                ...
                <CardContent className="p-0">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {rows.length === 0 && (
                                    <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground"><div className="flex flex-col items-center gap-2"><Tags className="h-10 w-10 text-muted-foreground/40" /><span>{t.no_rows}</span></div></td></tr>
                                )}
                                {rows.map((r, idx) => (
                                    <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/20 align-top">
                                        ...
                                        <td className="px-4 py-3 font-medium">
                                            {r.title}
                                            {r.is_locked && (
                                                <span className="inline-flex items-center gap-1 ms-2 rounded-full border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-600">
                                                    <Lock className="h-2.5 w-2.5" /> {t.locked_hint}
                                                </span>
                                            )}
                                        </td>
