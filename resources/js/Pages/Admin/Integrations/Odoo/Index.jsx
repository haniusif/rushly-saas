...
function VendorRow({ v, syncUrl, t }) {
    ...
    return (
        <tr className="border-t border-border">
            ...
            <td className="px-4 py-3">{v.odoo_partner_id || <span className="text-muted-foreground">—</span>}</td>
            <td className="px-4 py-3"><StatusPill status={v.odoo_sync_status} /></td>
...
function AddVendorForm({ url, t }) {
    ...
    const onSubmit = (e) => { e.preventDefault(); form.post(url, { preserveScroll: true, onSuccess: () => form.reset() }); };
    return (
        <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-3 mt-4 pt-4 border-t border-border">
            <Field label={t.courier_key}><Input value={form.data.courier_key} onChange={(e) => form.setData('courier_key', e.target.value)} placeholder="aramex" className="w-32" /></Field>
            <Field label={t.display_name}><Input value={form.data.display_name} onChange={(e) => form.setData('display_name', e.target.value)} placeholder="Aramex" className="w-48" /></Field>
...
export default function Index({ settings = {}, counts = {}, vendors = [], permissions = {}, urls = {}, t = {} }) {
    const form = useForm({
        ...
        default_invoice_journal_id: settings.default_invoice_journal_id ?? '',
        default_bill_journal_id:    settings.default_bill_journal_id ?? '',
        default_payment_journal_id: settings.default_payment_journal_id ?? '',
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        <Card>
                            <CardContent className="p-6 space-y-4">
                                ...
                                <Toggle label="Enable Odoo sync" value={form.data.enabled === '1'} onChange={(v) => form.setData('enabled', v ? '1' : '0')} />

                                <div className="grid gap-4 md:grid-cols-2">
                                    <Field label={t.host_url} error={form.errors.host_url} hint={t.host_url_hint}>
                                        <Input value={form.data.host_url} onChange={(e) => form.setData('host_url', e.target.value)} placeholder="https://mycompany.odoo.com" />
                                    </Field>
                                    <Field label={t.database} error={form.errors.database} hint={t.database_hint}>
                                        <Input value={form.data.database} onChange={(e) => form.setData('database', e.target.value)} placeholder="mycompany" />
                                    </Field>
                                    <Field label={t.username} error={form.errors.username}>
                                        <Input value={form.data.username} onChange={(e) => form.setData('username', e.target.value)} placeholder="admin@example.com" />
                                    </Field>
                                    <Field label={t.api_key} error={form.errors.api_key} hint={t.api_key_hint}>
                                        <Input value={form.data.api_key} onChange={(e) => form.setData('api_key', e.target.value)} className="font-mono text-sm" />
                                    </Field>
                                </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                    <Field label={t.invoice_journal_id} error={form.errors.default_invoice_journal_id}>
                                        <Input type="number" value={form.data.default_invoice_journal_id} onChange={(e) => form.setData('default_invoice_journal_id', e.target.value)} />
                                    </Field>
                                    <Field label={t.bill_journal_id} error={form.errors.default_bill_journal_id}>
                                        <Input type="number" value={form.data.default_bill_journal_id} onChange={(e) => form.setData('default_bill_journal_id', e.target.value)} />
                                    </Field>
                                    <Field label={t.payment_journal_id} error={form.errors.default_payment_journal_id}>
                                        <Input type="number" value={form.data.default_payment_journal_id} onChange={(e) => form.setData('default_payment_journal_id', e.target.value)} />
                                    </Field>
                                    ...
                                    <Field label={t.tax_id} error={form.errors.default_tax_id}>
                                        <Input type="number" value={form.data.default_tax_id} onChange={(e) => form.setData('default_tax_id', e.target.value)} />
                                    </Field>
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="bg-muted/40">
                                            <tr className="text-[11px] uppercase tracking-wide text-muted-foreground">
                                                ...
                                                <th className="text-start px-4 py-2">{t.partner_id}</th>
                            ...
                            <CardContent className="p-5 space-y-3">
                                ...
                                <div className="pt-3 border-t border-border text-[11px] text-muted-foreground space-y-1">
                                    {settings.ready
                                        ...
                                        : <p className="flex items-center gap-1"><XCircle className="h-3.5 w-3.5 text-amber-600" /> Missing connection or default IDs.</p>}
                                    {settings.cached_uid && <p>Cached UID: <code className="font-mono">{settings.cached_uid}</code></p>}
                                </div>
                            </CardContent>
                        </Card>
                    </aside>
