...
import { Package, Plus, Edit, Trash2, MoreVertical, ChevronLeft, ChevronRight, Image as ImageIcon } from 'lucide-react';
...
export default function Index({ rows = [], pagination = {}, currency = '', permissions = {}, urls = {}, t = {} }) {
        ...
        <AdminLayout title={t.title} breadcrumbs={[t.title, t.list]}>
            ...
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Package className="h-4 w-4" /><span>{showing}</span>
                </div>
                {permissions.create && (
                    <a href={urls.create} className="inline-flex h-9 items-center rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:bg-primary/90">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-border bg-muted/30 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                                    <th className="px-4 py-3 text-start w-14">#</th>
                                    <th className="px-4 py-3 text-start w-16">{t.image}</th>
                                ...
                                {rows.map((r, idx) => (
                                    <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                                        ...
                                        <td className="px-4 py-3">
                                            {r.image ? (
                                                <img src={r.image} alt={r.name} className="h-10 w-10 rounded object-cover border border-border" />
                                            ) : (
                                                <div className="h-10 w-10 rounded bg-muted grid place-items-center text-muted-foreground/60">
                                            ...
                                            <td className="px-4 py-3 text-end">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-44">
                                                        ...
                                                        {permissions.delete && (
                                                            <DropdownMenuItem onClick={() => del(r)} className="text-destructive focus:text-destructive">
