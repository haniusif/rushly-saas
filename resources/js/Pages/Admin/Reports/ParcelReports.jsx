...
import { Filter, Eraser, Calendar, Tag, Printer, User, Warehouse } from 'lucide-react';
...
export default function ParcelReports({
    rows = [], totals = {}, currency = '', has_data = false,
    filters = {}, lookups = {}, urls = {}, t = {},
}) {
    const [draft, setDraft] = React.useState({
        parcel_date:        filters.parcel_date        || '',
        parcel_merchant_id: filters.parcel_merchant_id || '',
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        router.get(urls.filter, draft, { preserveScroll: true, preserveState: true });
    };

    const toggleStatus = (val) => {
        const current = draft.parcel_status || [];
        const next = current.includes(val) ? current.filter((v) => v !== val) : [...current, val];
        set('parcel_status', next);
    };

    return (
        <AdminLayout title={t.title} breadcrumbs={[t.dashboard, t.reports, t.title]}>
            <Head title={t.title} />
                    ...
                    <form onSubmit={onSubmit} className="space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                            <div className="space-y-1.5">
                                ...
                                <Input
                                    value={draft.parcel_date}
                                    onChange={(e) => set('parcel_date', e.target.value)}
                                    placeholder={t.date_ph}
                                />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                    <User className="h-3 w-3" /> {t.merchant}
                                </label>
                                <Select
                                    value={draft.parcel_merchant_id}
                                    onChange={(e) => set('parcel_merchant_id', e.target.value)}
                                >
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                                    <Warehouse className="h-3 w-3" /> {t.hub}
                                </label>
                                <Select
                                    value={draft.hub_id}
                                    onChange={(e) => set('hub_id', e.target.value)}
                                >
                                    <option value="">{t.select_hub}</option>
                                    {(lookups.hubs || []).map((h) => (
                            ...
                            <div className="space-y-1.5 md:col-span-2 lg:col-span-4">
                                ...
                                <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto rounded-md border border-input bg-background px-2 py-2">
                                    {(lookups.statuses || []).map((s) => {
                                        const selected = (draft.parcel_status || []).includes(s.value);
                                        return (
                                            <button
                                                key={s.value}
                                                type="button"
                        ...
                        <div className="flex gap-2">
                            ...
                            {urls.print && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1.5 h-10 px-4 text-sm font-medium rounded-md border border-input bg-background hover:bg-muted/40 no-underline ml-auto"
                                >
