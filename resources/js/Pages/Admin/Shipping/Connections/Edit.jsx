...
export default function Edit({ mode = 'create', provider = {}, connection = null, fields = [], couriers = [], urls = {}, t = {} }) {
    ...
    const form = useForm({
        ...
        courier_delivery_man_id: connection?.courier_delivery_man_id ?? '',
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        <Card>
                            <CardContent className="p-6 space-y-4">
                                ...
                                {/* Which courier record represents this
                                    carrier. Setting it is what lets a
                                    successful shipment move the parcel to
                                    out-for-delivery; leaving it blank keeps
                                    the old behaviour of not touching status. */}
                                <Field label={t.courier} error={form.errors.courier_delivery_man_id} hint={t.courier_hint}>
                                    <select
                                        className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                        value={form.data.courier_delivery_man_id ?? ''}
                                        onChange={(e) => form.setData('courier_delivery_man_id', e.target.value)}
                                    >
                                        <option value="">{t.courier_none}</option>
                                        {couriers.map((c) => (
