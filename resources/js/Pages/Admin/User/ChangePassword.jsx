...
import { Lock, Eye, EyeOff, ArrowLeft, Save, ShieldCheck, Mail } from 'lucide-react';
...
function scorePassword(v = '') {
    ...
    if (s <= 2) return { level: 1, tone: 'rose' };
    if (s === 3) return { level: 2, tone: 'amber' };
    return { level: 3, tone: 'emerald' };
}

function PasswordField({ label, name, value, onChange, error, autoFocus }) {
...
function StrengthMeter({ value, t = {} }) {
    ...
    const toneBar = { rose: 'bg-rose-500', amber: 'bg-amber-500', emerald: 'bg-emerald-500' }[tone] || 'bg-muted';
    return (
        <div className="mt-2 flex items-center gap-2">
            <div className="flex gap-1 flex-1">
                {[1, 2, 3].map((b) => (
                    <span key={b} className={cn('h-1.5 flex-1 rounded-full transition-colors', b <= level ? toneBar : 'bg-muted')} />
                ))}
            </div>
            <span className={cn(
                ...
                <Card className="rounded-xl shadow-sm border border-border">
                    <CardContent className="p-6 space-y-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-0.5">
                                    {t.subheading?.replace(':name', user.name || '')}
                                </p>
                                <p className="text-xs text-muted-foreground mt-1.5">{t.intro}</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={submit} className="space-y-4">
                            ...
                            {/* Send-email toggle */}
                            <label className={cn(
                                'flex items-start gap-3 rounded-lg border border-border bg-muted/20 p-3',
                                t.no_email && 'opacity-60 cursor-not-allowed'
                            )}>
                                <input
                                    type="checkbox"
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="text-[11px] text-muted-foreground mt-0.5">{t.send_email_hint}</div>
                                </div>
                            </label>
