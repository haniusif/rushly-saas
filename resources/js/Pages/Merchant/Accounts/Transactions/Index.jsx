...
import { Filter, Eraser, Calendar, Tag, Wallet } from 'lucide-react';
...
const STATUS_TONE = {
    ...
    2: 'bg-emerald-50 text-emerald-700 border-emerald-200',// processed
    3: 'bg-rose-50    text-rose-700    border-rose-200',   // rejected
};
                ...
                <CardContent className="p-4">
                    <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={form.data.account} onChange={(e) => form.setData('account', e.target.value)}>
                                <option value="">{t.account_ph}</option>
                                {(lookups.accounts || []).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                            </Select>
                        </div>
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-muted/30 text-xs uppercase tracking-wide text-muted-foreground">
                                    <tr>
                                        ...
                                        <th className="text-start font-medium px-4 py-2.5">{t.account_details}</th>
                                        <th className="text-start font-medium px-4 py-2.5">{t.transaction_id}</th>
                                        <th className="text-start font-medium px-4 py-2.5 w-40">{t.request_date}</th>
                                        <th className="text-start font-medium px-4 py-2.5 w-32">{t.status}</th>
                                ...
                                <tbody className="divide-y divide-border">
                                    {rows.map((r) => (
                                        <tr key={r.id}>
                                            ...
                                            <td className="px-4 py-2.5">{r.account_label || <span className="text-muted-foreground">—</span>}</td>
                                            <td className="px-4 py-2.5 font-mono text-xs">{r.transaction_id || <span className="text-muted-foreground">—</span>}</td>
                                            <td className="px-4 py-2.5 text-xs text-muted-foreground">{r.date}</td>
                                            <td className="px-4 py-2.5">
                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border ${STATUS_TONE[r.status] || 'bg-muted/40 text-muted-foreground border-border'}`}>
                                                    {r.status_label}
                                                </span>
                                            </td>
