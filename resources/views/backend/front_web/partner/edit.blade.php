    ...
    <div class="container-fluid dashboard-content">
        <div class="tw-px-1 tw-pt-4 sm:tw-px-2">
            ...
            <nav class="tw-flex tw-items-center tw-gap-2 tw-text-xs tw-text-gray-500 tw-mb-4">
                ...
                <i class="fa fa-angle-right tw-text-[10px] tw-text-gray-400 tw-rtl-flip"></i>
                <a href="{{ route('partner.index') }}" class="hover:tw-text-brand-600 tw-no-underline">{{ __('levels.partner') }}</a>
                <i class="fa fa-angle-right tw-text-[10px] tw-text-gray-400 tw-rtl-flip"></i>
            ...
            <div class="tw-bg-white tw-border tw-border-gray-100 tw-rounded-xl tw-shadow-card tw-overflow-hidden">
                <div class="tw-px-6 tw-py-5 tw-border-b tw-border-gray-100 tw-flex tw-items-center tw-gap-3">
                    @if (@$partner->image)
                        <img src="{{ $partner->image }}" alt="" class="tw-w-10 tw-h-10 tw-rounded-lg tw-object-contain tw-bg-gray-50 tw-border tw-border-gray-100 tw-p-1" />
                    @endif
                    <div>
                        <h1 class="tw-text-lg tw-font-semibold tw-text-gray-900 tw-m-0">{{ __('levels.partner') }} {{ __('levels.edit') }}</h1>
                        <p class="tw-text-xs tw-text-gray-500 tw-mt-0.5 tw-m-0">{{ @$partner->name }}</p>
                    </div>
                </div>
                <form action="{{ route('partner.update', $partner->id) }}" method="POST" enctype="multipart/form-data" id="basicform">
                    ...
                    <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-5 tw-p-6">
                        ...
                        <div>
                            <label for="link" class="tw-block tw-text-sm tw-font-medium tw-text-gray-700 tw-mb-1.5">{{ __('levels.link') }} <span class="tw-text-red-500">*</span></label>
                    ...
                    <div class="tw-flex tw-items-center tw-justify-end tw-gap-2 tw-px-6 tw-py-4 tw-border-t tw-border-gray-100 tw-bg-gray-50">
                        <a href="{{ route('partner.index') }}" class="tw-inline-flex tw-items-center tw-h-10 tw-px-4 tw-text-sm tw-font-medium tw-text-gray-700 tw-bg-white hover:tw-bg-gray-100 tw-border tw-border-gray-200 tw-rounded-lg tw-no-underline">{{ __('levels.cancel') }}</a>
