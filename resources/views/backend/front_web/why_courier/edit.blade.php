        ...
        <div class="tw-px-1 tw-pt-4 sm:tw-px-2">
            ...
            <div class="tw-bg-white tw-border tw-border-gray-100 tw-rounded-xl tw-shadow-card tw-overflow-hidden">
                <div class="tw-px-6 tw-py-5 tw-border-b tw-border-gray-100 tw-flex tw-items-center tw-gap-3">
                    @if (@$whycourier->image)
                        <img src="{{ $whycourier->image }}" alt="" class="tw-w-10 tw-h-10 tw-rounded-lg tw-object-cover tw-bg-gray-100" />
                    @endif
                    <div>
                        <h1 class="tw-text-lg tw-font-semibold tw-text-gray-900 tw-m-0">{{ __('levels.why_courier') }} {{ __('levels.edit') }}</h1>
                        <p class="tw-text-xs tw-text-gray-500 tw-mt-0.5 tw-m-0">{{ @$whycourier->title }}</p>
                    </div>
                </div>
                <form action="{{ route('why.rushly.update', $whycourier->id) }}" method="POST" enctype="multipart/form-data" id="basicform">
                    @csrf
                    @method('put')
                    <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-5 tw-p-6">
                        <div>
                            ...
                            <input id="title" type="text" name="title" placeholder="{{ __('placeholder.Enter_title') }}" autocomplete="off"
                                   ...
                                   value="{{ old('title', @$whycourier->title) }}" required>
                        ...
                        <div>
                            ...
                            <input id="position" type="text" name="position" placeholder="{{ __('placeholder.Enter_Position') }}" autocomplete="off"
                                   ...
                                   value="{{ old('position', $whycourier->position) }}">
