...
@section('title')
    الشركات المزودة {{ __('levels.edit') }}
@endsection
@section('maincontent')
<div class="container-fluid dashboard-content">
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('supplier_companies.index') }}" class="breadcrumb-link">الشركات المزودة</a></li>
    ...
    <div class="row">
        <div class="col-xl-6 col-lg-6 col-md-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="pageheader-title">تعديل شركة مزودة</h2>
                    <form action="{{ route('supplier_companies.update') }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror">
                                @foreach(trans('status') as $key => $status)
                                    <option value="{{ $key }}" {{ (old('status', $supplier->status) == $key) ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                            @error('status') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <button type="submit" class="btn btn-primary">{{ __('levels.update') }}</button>
                        <a href="{{ route('supplier_companies.index') }}" class="btn btn-secondary">{{ __('levels.cancel') }}</a>
                    </form>
                </div>
            </div>
        </div>
